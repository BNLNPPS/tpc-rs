...
// -----------------------------------------------------------------
// bfc/.make/db/.const/StarDb/tpc/tsspars/.tsspar/tsspar Allocated rows: 1  Used rows: 1  Row size: 240 bytes
//  Table: tss_tsspar_st[0]--> tss_tsspar_st[0]
// ====================================================================
// ------  Test whether this table share library was loaded ------
  if (!TClass::GetClass("St_tss_tsspar")) return 0;
tss_tsspar_st row;
...
memset(&row,0,tableSet->GetRowSize());
 memcpy(&row.fileout,"\x04\x08o\xfc\x00",5);// output file for pixel data (none->table) 
    ...
    row.gain_in	 =       3558; // gas gain:inner sector (dimensionless) ;
    row.gain_out	 =       1310; // gas gain:outer sector (dimensionless) ;
    ...
    row.threshold	 =          0; // adc threshold (adc counts but a real num ;
