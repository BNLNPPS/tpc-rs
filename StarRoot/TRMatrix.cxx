...
TRMatrix::TRMatrix(const TRMatrix &S, Int_t NI, Int_t NJ, Int_t I, Int_t J)
{
  ...
  if (I == 0) {
    ::Error("TRMatrix::TRMatrix(const TRMatrix &)", "index i %d out of bounds (size: %d, this: %p)",
            I, S.NI(), this); I = 1;
  }

  if (J == 0) {
    ::Error("TRMatrix::TRMatrix(const TRMatrix &)", "index j %d out of bounds (size: %d, this: %p)",
            J, S.NJ(), this); J = 1;
  }
...
std::ostream &operator<<(std::ostream &s, const TRMatrix &target)
{
  ...
  if (Array) {
    ...
    for (int i = 0; i < Nrows; i++) {
      ...
      for (int j = Ncols - 1; j >= 0; j--) if (Array[j + i * Ncols] == 0.0) {Nzeros++;}
        else break;
